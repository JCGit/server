...
void netioUnregisterIdle(EventBase* base, const IdleId& idle);
void netioUpdateIdle(EventBase* base, const IdleId& idle);
...
void TcpConn::cleanup(const TcpConnPtr& con) {
    ...
    for (auto& idle: idleIds_) {
        netioUnregisterIdle(getBase(), idle);
    }
